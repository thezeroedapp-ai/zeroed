...
const { getDb, getGoals, createGoal, deleteGoal } = require('../db/database');
...
router.get('/', (req, res) => {
  try {
    const goals = getGoals(1);
    const db    = getDb();
    const user  = db.prepare('SELECT * FROM users WHERE id = 1').get();
    const accounts = db.prepare(ACCOUNTS_QUERY).all();
...
router.delete('/:id', (req, res) => {
  try {
    deleteGoal(parseInt(req.params.id), 1);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
