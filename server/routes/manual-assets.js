...
router.post('/', async (req, res) => {
  try {
    const { name, asset_type, asset_subtype, current_value, linked_loan_id } = req.body;
    ...
    const id = await db.addManualAsset(req.user.uid, {
      name,
      asset_type,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await db.updateManualAsset(req.user.uid, req.params.id, {
      name,
      asset_type,
      asset_subtype: asset_subtype || null,
      current_value: Math.round(current_value * 100) / 100,
